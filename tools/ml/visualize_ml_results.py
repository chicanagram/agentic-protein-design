...
def _plot_data_efficiency(
    ...
    model_order = [str(x) for x in pd.unique(df_plot["plot_model_name"].astype(str))]
    feature_seen = [str(x) for x in pd.unique(df_plot["feature_label"].astype(str))]
    ...
    for feature_label in feature_order:
        for model_name in model_order:
            grp = df_plot[
                (df_plot["feature_label"].astype(str) == feature_label)
                & (df_plot["plot_model_name"].astype(str) == model_name)
            ]
            if grp.empty:
                continue
