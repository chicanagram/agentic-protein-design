...
def run_jackhmmer(query_fasta, db_name, output_file, e_thres=1e-5, incE_thres=1e-5, max_target_seqs=None, num_cpu=None, query_dir='', db_dir='', output_dir='', outfmt=0):
    """
    Run JackHMMER against a local FASTA database.

    Returns:
        Path to the written JackHMMER output file.
    """
    ...
    try:
        ...
        print(
            f"Jackhmmer search completed ({round((end - start) / 60, 3)} min). Results saved in {output_path}")
