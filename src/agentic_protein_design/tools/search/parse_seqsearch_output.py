...
def parse_hmmer_output(input_fname, output_fname, output_fasta_fname, query_seq_input, sequences_dir, seqsearch_dir, max_target_seqs=None, parse_description=False):
    """
    Parse PHMMER/JackHMMER text output into CSV and FASTA artifacts.

    Returns:
        Parsed hits DataFrame.
    """
    seqsearch_dir = Path(seqsearch_dir)
    sequences_dir = Path(sequences_dir)
    input_path = Path(input_fname)
    if not input_path.is_absolute():
        input_path = seqsearch_dir / input_path
    output_csv_path = Path(output_fname)
    if not output_csv_path.is_absolute():
        output_csv_path = seqsearch_dir / output_csv_path
    output_fasta_path = Path(output_fasta_fname)
    if not output_fasta_path.is_absolute():
        output_fasta_path = sequences_dir / output_fasta_path
    if output_fasta_path.suffix.lower() != ".fasta":
        output_fasta_path = output_fasta_path.with_suffix(".fasta")
    ...
    if isinstance(query_seq_input, str):
        query_seq, query_seq_name, _ = fetch_sequences_from_fasta(str(sequences_dir / query_seq_input))
    elif isinstance(query_seq_input, tuple):
        (query_seq, query_seq_name) = query_seq_input
        if isinstance(query_seq, str):
            query_seq = [query_seq]
        if isinstance(query_seq_name, str):
            query_seq_name = [query_seq_name]
    _ = write_sequence_to_fasta(query_seq+sequences, query_seq_name+seq_names, output_fasta_path.stem, f"{output_fasta_path.parent}/")
    return df
...
def parse_blastp_output(input_fname, output_fname, output_fasta_fname, query_seq_input, sequences_dir, seqsearch_dir, max_target_seqs=None, parse_description=False):
    """
    Parse BLASTP text output into CSV and FASTA artifacts.
